...
setup('authenticate with Keycloak as user with roles', async ({ page }) => {
	...
	await page.fill(
		'input#username',
		process.env.NEXT_KEYCLOAK_USER ?? 'withrole@withrole.com',
	);
	await page.fill(
		'input#password',
		process.env.NEXT_KEYCLOAK_PASSWORD ?? 'withrole',
	);
	...
	await expect(
		page.locator('button').filter({
			hasText: process.env.KEYCLOAK_USER ?? 'withrole@withrole.com',
		}),
	).toBeVisible();
