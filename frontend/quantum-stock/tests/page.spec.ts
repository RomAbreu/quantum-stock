...
test.describe('Homepage - authenticated with permission', () => {
	...
	test('should display full navigation menu for users with permissions', async ({
		page,
	}) => {
...
test.describe('Homepage - authenticated without permission', () => {
	...
	test('should display simplified UI for users without permissions', async ({
		page,
	}) => {
		// Navigation items should NOT be visible
		await expect(
			page.locator('a:has-text("Dashboard")').first(),
		).not.toBeVisible();
		await expect(page.locator('a:has-text("Stock")').first()).not.toBeVisible();

		// User profile should be visible for logout
		await expect(
			page.locator('button:has-text("withoutrole@withoutrole.com")').first(),
		).toBeVisible();
	});

	test('should display simplified welcome message for users without permissions', async ({
		page,
	}) => {
		await expect(
			page.locator('h1:has-text("Quantum Stock")').first(),
		...
		await expect(
			page.locator('p:has-text("Bienvenido a Quantum Stock")').first(),
		).toBeVisible();
	});
});
