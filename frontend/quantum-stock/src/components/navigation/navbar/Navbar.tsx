...
export default function Navbar(props: Readonly<NavbarProps>) {
    ...
    const isRegularUser = useMemo(() => {
        return keycloak.authenticated && !hasPermission;
    }, [keycloak.authenticated, hasPermission]);

    const userNavItems = useMemo(() => {
        if (isRegularUser) {
            return navbarItems.filter(item => 
                item.href.includes('stock') || item.href.includes('dashboard')
            );
        }
        return navbarItems;
    }, [isRegularUser]);
    ...
    return (
        ...
        >
            ...
            <NavbarContent className="hidden sm:flex" justify="center">
                {isAuthenticated ? (
                    <>
                        {(hasPermission ? navbarItems : userNavItems).map((item) => (
            ...
            <NavbarContent className="hidden sm:flex" justify="end">
                {!isAuthenticated ? (
                    ...
                    <>
                        ...
                        {/* Notificaciones - Solo para admin y employee */}
                        {hasPermission && (
                            <NavbarItem>
                                <Dropdown placement="bottom-end">
                                    <DropdownTrigger>
